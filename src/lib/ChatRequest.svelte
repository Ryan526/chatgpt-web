...
export class ChatRequest {
    ...
    async sendRequest (messages: Message[], opts: ChatCompletionOpts, overrides: ChatSettings = {} as ChatSettings): Promise<ChatCompletionResponse> {
      ...
      const fileMessages = messages.filter(m => m.role === 'file' && m.image && m.image.b64image);
      if (fileMessages.length > 0) {
        for (const fileMessage of fileMessages) {
          // Process the file as needed. For example, send it to an API or handle locally.
          // Here, we'll assume you want to send the file content as part of the message.
          // You might need to adjust this based on your backend API.

          // Example: Append file content to the prompt or handle accordingly
          opts.prompt = opts.prompt || '';
          opts.prompt += `\nFile uploaded: ${fileMessage.content}`;
        }
      }
      ...
      const includedRoles = ['user', 'assistant', 'file'].concat(chatSettings.useSystemPrompt ? ['system'] : []);
