<script lang="ts">
  ...
  import { autoGrowInputOnEvent, scrollToBottom, sizeTextElements, readFileAsBase64 } from './Util.svelte';
  ...
  const handleFiles = async (files: FileList) => {
    ...
    for (const file of fileArray) {
      ...
      const fileMessage: Message = {
        ...
        content: file.name,
        uuid: uuidv4(),
        image: { // Reusing the 'image' field to store file data
          id: uuidv4(),
          b64image: base64,
          chats: [chatId]
        },
  ...
  >
    ...
    <Footer class="prompt-input-container" strongMask={true}>
      <form class="field has-addons has-addons-right is-align-items-flex-end" on:submit|preventDefault={() => submitForm()}>
        ...
        {/if}
        <p class="control upload">
          <button title="Upload Files" class="button is-info" type="button" on:click={triggerFileSelect}><span class="icon"><Fa icon={faUpload} /></span></button>
        </p>
      </form>
...
<style>
  ...
  .prompt-input-container .control.upload {
    display: flex;
    align-items: center;
  }

  /* Optional: Adjust the upload button style */
  .prompt-input-container .control.upload .button {
    background-color: #00d1b2;
    color: white;
  }

  .prompt-input-container .control.upload .button:hover {
