<script context="module" lang="ts">
  ...
  export type RequestType = 'chat' | 'instruct' | 'image' | 'file';
  ...
  export interface UploadedFile {
      ...
      b64file: string; // Base64 encoded file content
      chats: number[];
    }

  export type Message = {
      role: 'user' | 'assistant' | 'system' | 'error' | 'image' | 'file';
      content: string;
      uuid: string;
      usage?: Usage;
      model?: Model;
      ...
      streaming?: boolean;
      image?: ChatImage;
      file?: UploadedFile; // Added for file uploads
      files?: UploadedFile[]; // Supports multiple file uploads
      created?: number;
      skipOnce?: boolean;
      appendOnce?: string[];
    };
